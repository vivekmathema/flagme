...
multipleAlignment<-function(pd,group,bw.gap=0.8,wn.gap=0.6,bw.D=.20,wn.D=.05,filterMin=3,lite=FALSE,usePeaks=TRUE,df=50,verbose=TRUE,timeAdjust=FALSE,doImpute=FALSE) {
  groups<-unique(group)
  ...
  for(i in 1:length(groups)) {
	...
	if (doImpute)
	  if(timeAdjust)
	    ...
	    impute[[i]]<-imputePeaks(pd,pAs[[i]],type=1)
  }
  names(cAs)<-groups
  names(pAs)<-groups
  if(length(groups)==1) {
    ...
    wRA<-new("betweenAlignment",mergedPeaksDataset=new("peaksDataset"),ind=pAs[[1]]@merges[[pn]]$ind,runs=pAs[[1]]@merges[[pn]]$runs,
	                            imputeind=list(),filtind=list(),newind=list(),cA=cAs[[1]],pA=pAs[[1]],
								groups=as.character(rep(groups,nrow(cAs[[1]]@dist))))
  } else {
    wRA<-betweenAlignment(pd,cAs,pAs,impute,gap=bw.gap,D=bw.D,filterMin=filterMin,usePeaks=usePeaks,df=df,verbose=verbose)
  }
  
  ma<-new("multipleAlignment",clusterAlignmentList=cAs,progressiveAlignmentList=pAs,
                              timeDiff=timedf,impute=impute,betweenAlignment=wRA)
  if (lite) {
...
function(object) {
   ...
   cat(length(object@clusterAlignmentList), "groups:",sapply(object@progressiveAlignmentList,FUN=function(u) length(u@merges)+1),"samples, respectively.\n",sep=" ")
...
imputePeaks<-function(pD,obj,type=1,obj2=NULL,filterMin=3,verbose=TRUE) {
  ...
  if (type==1) {  # impute from scan numbers
  ...
  } else if (type==2) {  # impute from scan numbers
