...
progressiveAlignment<-function(pD,cA,D=1000,gap=.5,verbose=TRUE,usePeaks=TRUE,df=30,compress=TRUE) {
  m<-cA@merge
  ...
  for(i in 1:nrow(m)) {
	...
	merges[[i]]<-list(ind=new.ind,mi=mi,runs=c(left.runs,right.runs),left=left.runs,right=right.runs,r=sim,compressed=FALSE)
  }
  cA<-NULL
  if (verbose)
    print(gc())
