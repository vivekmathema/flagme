...
clusterAlignment<-function(pD,runs=1:length(pD@rawdata),timedf=NULL,usePeaks=TRUE,verbose=TRUE,...) {
  n<-length(runs)
  if (usePeaks)
  ...
  for(i in 1:(n-1)) {
    ...
    for(j in (i+1):n) {
	  ...
	  count<-count+1
      if (verbose)
        cat("[clusterAlignment] Aligning",names(pD@rawdata)[run.i],"to",names(pD@rawdata)[run.j],"\n")
	  if (usePeaks)
	    alignments[[count]]<-peaksAlignment(pD@peaksdata[[run.i]],pD@peaksdata[[run.j]],pD@peaksrt[[run.i]],pD@peaksrt[[run.j]],usePeaks=usePeaks,timedf=timedf[[count]],verbose=verbose,...)
	  else
	    alignments[[count]]<-peaksAlignment(pD@rawdata[[run.i]],pD@rawdata[[run.j]],pD@rawrt[[run.i]],pD@rawrt[[run.j]],usePeaks=usePeaks,timedf=NULL,verbose=verbose,...)
  ...
  new("clusterAlignment",runs=runs,aligned=aligned,gap=alignments[[1]]@gap,D=alignments[[1]]@D,dist=dist,alignments=alignments,merge=merge)
}
