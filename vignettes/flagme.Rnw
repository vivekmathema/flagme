...
\texttt{flagme} package, which is designed to process, visualize and
statistically analyze sets of GCMS samples.  The ideas discussed here
were originally designed with GCMS-based metabolomics in mind, but
indeed some of the methods and visualizations could be useful for
LC-MSMS datasets.  The {\em fragment-level analysis} though, takes
advantage of the rich fragmentation patterns observed from electron
impact ionization. 

There are many aspects of data processing for GCMS data.  Generally,
...
installed.  This data package contains several GCMS samples from a
...
create vectors of the filenames and create a \texttt{peakDataset}
...
analogous \texttt{addChromaTOFPeaks} function). Support for XMCS or
MzMine may be added in the future. Ask the author if another detection
...
ion currents (TIC), or extracted ion currents (XICs). To view TICs,
you can call:  
...
labeled with an integer index. One of  the main challenges is to match
...
One of the first challenges of GCMS data is the matching of detected
...
that a gap penalty in the middle range (0.4-0.6) works well for GCMS
...
investigation of GCMS data, but have not made part of the package just
yet.  Some of the most useful procedures will be released, such as: 

\begin{enumerate}

\item Parsers for other peak detection algorithms (e.g. XCMS, MzMine)
  and parsers for other alignment procedures (e.g. SpectConnect) and
  perhaps retention indices procedures. 
